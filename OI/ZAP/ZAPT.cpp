...
int main(){
	...
	for (int i=0; i<n; ++i) {
		...
		if (glow.size() == zapV.size()) {
			if (segs[i].s >= koniec &&  segs[i].s >= koniecZastepcy) {
		...
		else if (glow.size() < zapV.size() && segs[i].s >= koniec && segs[i].s >= koniecZastepcy) {
			poprzedniKoniec = koniec;
			koniec = segs[i].e;
			...
			if(segs[i].s >= poprzedniKoniec && zap.count(glow.back()) == 0) {
			...
			else if (segs[i].s >= koniecZastepcy && segs[i].s >= koniec) {
				poprzedniKoniec = koniec;
				koniec = segs[i].e;
